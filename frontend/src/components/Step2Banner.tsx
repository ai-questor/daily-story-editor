...
export default function Step2Banner({ result, productImage, setProductImage, loading, onSubmit }: Props) {
  return (
    <div className="card p-4 shadow-sm mt-3">
      ...
      {result && (
        <>
          ...
          {result.captions.map((c, i) => <p key={i} className="alert alert-secondary">{c}</p>)}
      ...
      {productImage && (
        <div className="mb-3">
          <img src={URL.createObjectURL(productImage)} alt="미리보기" className="img-thumbnail" style={{ maxWidth: "200px" }} />
        </div>
      )}
